...
def additive_attn(q, k, v):
    """
    Inputs:
    q - query - [1 x h_q] -> # TODO can we expand to [n' x h_q]
    k - keys - [n x h_k]
    v - values associated with keys - [n x h_v]

    Desired outcome:
    Apply attention for q, over k, to get a weighted sum of vs
    where attention = ffn_2(tanh(n * [ffn_1a(q)] + ffn_1b(k))) where + is element-wise 
    ffn is usually ff_2(nonlinearity(ff_1(x))) where ffn_1 is a axb matrix, ffn_2 is a bxa matrix
    ffn_1a is [h_q x d], ffn_1b is [h_k x d], ffn_2 is [d x 1]
    n * [ffn_1a(q)] -> n x d
    ffn_1b(k) -> n x d
    tanh(n * [ffn_1a(q)] + ffn_1b(k)) - > n x d
    ffn_2(tanh(n * [ffn_1a(q)] + ffn_1b(k))) - > n x 1
    -> output n x 1
    and then apply softmax and multiply associated probability by v and sum
    """
    pass
